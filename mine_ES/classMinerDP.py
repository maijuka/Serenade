...
####################################################
    ...
    def doExpansions(self, cust_params={}):
        ...
        for i in range(self.constraints.getCstr("nb_extension_rounds")): 
            ...
            if self.P:
                best_red = self.P.pop(max(self.P.keys()))
                r = self.M.pop(best_red)[0]  # [0] is the red, [1] is the age
                Q.append(r)
                Q.remove(best_red)

                for k, v in list(self.P.items()):
                    if self.M[v][1] >= self.age_threshold:
                        self.M.pop(v)
                        self.P.pop(k)
                    else:
                        self.M[v][1] += 1
        ...
        for r in Q:
            r.setPrivateSuppSizes((self.q_budget/len(Q))/2)
    ...
    def expandRedescriptions(self, nextge, rcollect=None):
        ...
        for red in nextge:
            ...
            if self.charbon.getStore():
                best = self.charbon.getStore().getFoundRedsKey(self.data)
                key = [*best][0]

                self.M[red] = [best[key], 1]
                self.P[key] = red
